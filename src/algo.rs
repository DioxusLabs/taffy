...
pub fn compute(root: &style::Node) -> layout::Node {
  let result = compute_internal(root, FlexSize::min(), FlexSize::max());

  layout::Node {
    width: if root.flex_direction.is_row() { result.size.main } else { result.size.cross },
    height: if root.flex_direction.is_column() { result.size.main } else { result.size.cross },
    x: 0.0,
    y: 0.0,
...
fn compute_internal(node: &style::Node, min: FlexSize, max: FlexSize) -> ComputeResult {
  // 9.2. Line Length Determination

  // 1. Generate anonymous flex items as described in §4 Flex Items.

  let mut flex_items: Vec<FlexItem> = node.children.iter().map(|child| { 
    FlexItem { 
      node: child, 
  ...
  let avaliable_main = node.main_size().resolve(max.main, {
    max.main 
        - node.main_margin_start().resolve(max.main, 0.0) 
        - node.main_margin_end().resolve(max.main, 0.0) 
        - node.main_padding_start().resolve(max.main, 0.0) 
        - node.main_padding_end().resolve(max.main, 0.0) 
        - node.main_border_start().resolve(max.main, 0.0)
        - node.main_border_end().resolve(max.main, 0.0)
  });

  let avaliable_cross = node.cross_size().resolve(max.cross, {
    max.cross 
        - node.cross_margin_start().resolve(max.cross, 0.0) 
        - node.cross_margin_end().resolve(max.cross, 0.0) 
        - node.cross_padding_start().resolve(max.cross, 0.0) 
        - node.cross_padding_end().resolve(max.cross, 0.0) 
        - node.cross_border_start().resolve(max.cross, 0.0)
        - node.cross_border_end().resolve(max.cross, 0.0)
  });
  ...
  for child in &mut flex_items {
    ...
    let flex_basis = child.node.flex_basis.resolve(avaliable_main, f32::INFINITY);
    ...
    if let Some(ratio) = child.node.aspect_ratio {
      let cross_size = node.cross_size().resolve(avaliable_cross, f32::INFINITY);

      if cross_size.is_finite() && child.node.flex_basis == style::Dimension::Auto {
    ...
    if min.main.is_finite() || max.main.is_finite() {
      let size = compute_internal(child.node, min, max).size;
      child.flex_basis = size.main;
      break;
    }
    ...
    if avaliable_main.is_infinite() {
      let size = compute_internal(child.node, FlexSize::min(), FlexSize::max()).size;
      child.flex_basis = size.main;
      break;
    }
    ...
    let size = compute_internal(child.node, FlexSize::min(), FlexSize { main: avaliable_main, cross: avaliable_cross }).size;
    child.flex_basis = size.main;
  }
  ...
  for child in &mut flex_items {
    child.hypothetical_inner_main_size = child.flex_basis
        .max(child.node.min_main_size().resolve(avaliable_main, f32::MIN))
        .min(child.node.max_main_size().resolve(avaliable_main, f32::MAX))
        .max(0.0);

    child.hypothetical_outer_main_size = child.hypothetical_inner_main_size
        + node.main_margin_start().resolve(avaliable_main, 0.0) 
        + node.main_margin_end().resolve(avaliable_main, 0.0);
  }
  ...
  for line in &mut flex_lines {
    ...
    for child in line.items.iter() {
      used_space += child.hypothetical_outer_main_size;
    }

    let growing = used_space < avaliable_main;
    ...
    let used_space: f32 = line.items.iter().map(|child| {
      if child.frozen {
    ...
    loop {
      ...
      let used_space: f32 = Iterator::chain(frozen.iter(), unfrozen.iter()).map(|child| {
        if child.frozen {
          child.target_main_size
        } else {
          child.flex_basis
      ...
      if free_space > 0.0 && growing {
        for child in &mut unfrozen {
          child.target_main_size = child.flex_basis + free_space * (child.node.flex_grow / sum_flex_grow);
        }
      } else if free_space < 0.0 && shrinking {
        for child in &mut unfrozen {
          child.target_main_size = child.flex_basis + free_space * (child.node.flex_shrink / sum_flex_shrink);
        }
      }
      ...
      for child in &mut unfrozen {
        let max = child.node.max_main_size().resolve(avaliable_main, f32::MAX);
        let min = child.node.min_main_size().resolve(avaliable_main, f32::MIN).max(0.0);
  ...
  for line in &mut flex_lines {
    for child in &mut line.items {
      let size = compute_internal(
        child.node, 
        FlexSize { main: child.target_main_size, cross: min.cross }, 
        FlexSize { main: child.target_main_size, cross: avaliable_cross },
      ).size;

      child.hypothetical_inner_cross_size = size.cross
          .max(child.node.min_cross_size().resolve(avaliable_cross, f32::MIN))
          .min(child.node.max_cross_size().resolve(avaliable_cross, f32::MAX))
          .max(0.0);

      child.hypothetical_outer_cross_size = child.hypothetical_inner_cross_size
          + node.cross_margin_start().resolve(avaliable_cross, 0.0) 
          + node.cross_margin_end().resolve(avaliable_cross, 0.0);
    }
  }
  ...
  if flex_lines.len() == 1 && avaliable_cross.is_finite() {
    flex_lines[0].cross_size = avaliable_cross.min(max.cross).max(min.cross);
  } else {
    for line in &mut flex_lines {
  ...
  if node.align_content == style::AlignContent::Stretch && avaliable_cross.is_finite() {
    let total_cross: f32 = flex_lines.iter().map(|line| line.cross_size).sum();

    if total_cross < avaliable_cross {
      let remaining = avaliable_cross - total_cross;
  ...
  for line in &mut flex_lines {
    for child in &mut line.items {
      ...
      if is_stretch && 
         child.node.cross_margin_start() != style::Dimension::Auto && 
         child.node.cross_margin_end() != style::Dimension::Auto &&
         child.node.cross_size() == style::Dimension::Auto {
        child.target_cross_size = line.cross_size
            .min(child.node.max_cross_size().resolve(avaliable_cross, f32::MAX))
            .max(child.node.min_cross_size().resolve(avaliable_cross, f32::MIN));
      } else {
      ...
      if is_stretch {
        let size = FlexSize { main: child.target_main_size, cross: child.target_cross_size };
        compute_internal(child.node, size, size);
      }
    }
  }
  ...
  for line in &mut flex_lines {
    ...
    if free_space > 0.0 {
      ...
      for child in &mut line.items {
        if child.node.main_margin_start() == style::Dimension::Auto {
          num_auto_margins += 1;
        }
        if child.node.main_margin_end() == style::Dimension::Auto {
          num_auto_margins += 1;
        }
      }
      ...
      if num_auto_margins > 0 {
        ...
        for child in &mut line.items {
          ...
          child.offset_main = match node.justify_content {
            ...
            style::JustifyContent::Center => if is_first { free_space / 2.0 } else { 0.0 },
            style::JustifyContent::FlexEnd => if is_first { free_space } else { 0.0 },
            style::JustifyContent::SpaceBetween => if is_first { 0.0 } else { free_space / (num_items - 1) as f32 },
  ...
  for line in &mut flex_lines {
    for child in &mut line.items {
      if child.target_cross_size < line.cross_size {
        ...
        if child.node.cross_margin_start() == style::Dimension::Auto && child.node.cross_margin_end() == style::Dimension::Auto {
          child.cross_margin_start = free_space / 2.0;
          child.cross_margin_end = free_space / 2.0;
        } else if child.node.cross_margin_start() == style::Dimension::Auto {
          child.cross_margin_start = free_space;
        } else if child.node.cross_margin_end() == style::Dimension::Auto {
          child.cross_margin_end = free_space;
        } else {
          child.cross_margin_start = child.node.cross_margin_start().resolve(line.cross_size, 0.0);
          child.cross_margin_end = child.node.cross_margin_end().resolve(line.cross_size, 0.0);
  ...
  let total_cross_size = flex_lines.iter().map(|line| line.cross_size).sum();
  let container_cross_size = if avaliable_cross.is_finite() {
    avaliable_cross
  } else {
    total_cross_size
  }.min(max.cross).max(min.cross);;
  ...
  for line in &mut flex_lines {
    line.offset_cross = match node.align_content {
      ...
      style::AlignContent::FlexEnd => if is_first { free_space } else { 0.0 },
      style::AlignContent::Center => if is_first { free_space / 2.0 } else { 0.0 },
      style::AlignContent::Stretch => 0.0,
      style::AlignContent::SpaceBetween => if is_first { 0.0 } else { free_space / (num_lines - 1) as f32 },
      style::AlignContent::SpaceAround => if is_first { (free_space / num_lines as f32) / 2.0 } else { free_space / num_lines as f32 },
    };
    is_first = false;
  }
  ...
  for line in &mut flex_lines {
    for child in &mut line.items {
      let size = FlexSize { main: avaliable_main, cross: container_cross_size };
      let result = compute_internal(child.node, size, size);

      // TODO incorporate margins, paddings, borders, and sublings
      children.push(layout::Node {
        width: if child.node.flex_direction.is_row() { result.size.main } else { result.size.cross },
        height: if child.node.flex_direction.is_column() { result.size.main } else { result.size.cross },
        x: if child.node.flex_direction.is_row() { child.offset_main } else { child.offset_cross + line.offset_cross },
        y: if child.node.flex_direction.is_column() { child.offset_main } else { child.offset_cross + line.offset_cross },
        children: result.children,
      });
    }
  }
