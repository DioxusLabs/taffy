...
use crate::tree::{
    LayoutFlexboxContainer, LayoutPartialTreeExt, NodeId, OofPositioningArea, StaticEdge, StaticPosition,
};
...
fn collect_oof_candidates(
    ...
    let main_start_anchor = (content_box_inset.main_start(dir), StaticEdge::Start);
    let main_end_anchor = (container_size.main(dir) - content_box_inset.main_end(dir), StaticEdge::End);
    let main_center_anchor = (
        (container_size.main(dir) + content_box_inset.main_start(dir) - content_box_inset.main_end(dir)) / 2.0,
        StaticEdge::Center,
    );
    let cross_start_anchor = (content_box_inset.cross_start(dir), StaticEdge::Start);
    let cross_end_anchor = (container_size.cross(dir) - content_box_inset.cross_end(dir), StaticEdge::End);
    let cross_center_anchor = (
        (container_size.cross(dir) + content_box_inset.cross_start(dir) - content_box_inset.cross_end(dir)) / 2.0,
        StaticEdge::Center,
    );
    ...
    for order in 0..tree.child_count(node) {
        ...
        let (main_anchor, main_edge) = match (main_keyword, main_axis_flex_start_reversed) {
            (AlignContentKeyword::SpaceBetween, false)
            | (AlignContentKeyword::Stretch, false)
            | (AlignContentKeyword::FlexStart, false)
            | (AlignContentKeyword::FlexEnd, true) => main_start_anchor,
            ...
            | (AlignContentKeyword::SpaceBetween, true) => main_end_anchor,
            (AlignContentKeyword::SpaceEvenly, _)
            | (AlignContentKeyword::SpaceAround, _)
            | (AlignContentKeyword::Center, _) => main_center_anchor,
        };
        ...
        let cross_anchor_for = |keyword: AlignItemsKeyword| {
            ...
            match (keyword, cross_axis_flex_start_reversed) {
                ...
                | (AlignItemsKeyword::FlexEnd, true) => cross_start_anchor,
                (AlignItemsKeyword::Stretch | AlignItemsKeyword::FlexStart, true)
                | (AlignItemsKeyword::FlexEnd, false) => cross_end_anchor,
                (AlignItemsKeyword::Center, _) => cross_center_anchor,
        ...
        let (cross_anchor, cross_edge) = cross_anchor_for(align_self.keyword);
        let cross = if matches!(align_self.safety, AlignmentSafety::Safe) {
            let (fallback_anchor, fallback_edge) = cross_anchor_for(resolve_self_alignment_safety(align_self, true));
            StaticPosition {
                anchor: cross_anchor,
                edge: cross_edge,
                fallback_anchor,
                fallback_edge,
                safe_extent: Some(container_size.cross(dir) - content_box_inset.cross_axis_sum(dir)),
            }
        } else {
            StaticPosition::from_edge(cross_anchor, cross_edge)
        };
        let main = StaticPosition::from_edge(main_anchor, main_edge);
