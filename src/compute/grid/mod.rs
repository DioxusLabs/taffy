    ...
    tree.set_detailed_grid_info(
        ...
        DetailedGridInfo {
            rows: DetailedGridTracksInfo::from_grid_tracks_and_track_count(
                final_row_counts,
                rows,
                finalize_line_names(detailed_row_line_names, final_row_counts, false),
            ),
            columns: DetailedGridTracksInfo::from_grid_tracks_and_track_count(
                final_col_counts,
                columns,
                finalize_line_names(detailed_column_line_names, final_col_counts, direction.is_rtl()),
            ),
...
/// Convert explicit-grid-relative line names into final-grid line names: pad with (unnamed)
/// implicit grid lines and mirror the line order for RTL columns to match the visual track
/// order used by [`DetailedGridTracksInfo::positions`]
#[cfg(feature = "detailed_layout_info")]
fn finalize_line_names<S: CheapCloneStr>(
    ...
    // Mirroring must match `reverse_non_gutter_tracks`: when the explicit grid has 0/1 tracks
    // all tracks are reversed (if there are at least 2 of them), otherwise only the explicit
    // tracks are reversed in-place
    let reverse_whole_grid = is_rtl && track_counts.explicit <= 1 && track_counts.len() >= 2;
    let reverse_explicit_span = is_rtl && track_counts.explicit >= 2;

    if !is_rtl && negative_implicit == 0 && explicit_line_names.line_count() == total_line_count {
    ...
    for line_index in 0..total_line_count {
        line_names.start_line();
        let source_index = if reverse_whole_grid {
            let unmirrored_index = total_line_count - 1 - line_index;
            (unmirrored_index >= negative_implicit && unmirrored_index < negative_implicit + explicit_line_count)
                .then(|| unmirrored_index - negative_implicit)
        } else if line_index >= negative_implicit && line_index < negative_implicit + explicit_line_count {
            let explicit_index = line_index - negative_implicit;
            if reverse_explicit_span {
                Some(explicit_line_count - 1 - explicit_index)
            } else {
                Some(explicit_index)
            }
        } else {
        ...
        if let Some(source_index) = source_index {
            for name in explicit_line_names.line(source_index) {
