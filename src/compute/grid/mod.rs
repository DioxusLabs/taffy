...
pub fn compute_grid_layout<Tree: LayoutGridContainer>(
    ...
    (0..tree.child_count(node)).for_each(|index| {
        ...
        if child_style.position().is_out_of_flow() {
            ...
            // coinciding with the grid's content edges) which bubbles up to the containing
            // block. Grid-placement properties do not apply when the grid is not the
            // containing block.
            // https://www.w3.org/TR/css-grid-1/#static-position
            if !containing_block_claims.for_position(child_style.position()) {
