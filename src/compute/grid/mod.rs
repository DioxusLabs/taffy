...
use crate::tree::{
    ...
    OofPositioningArea, RunMode, SizingMode, StaticEdge, StaticPosition,
};
...
pub fn compute_grid_layout<Tree: LayoutGridContainer>(
    ...
    (0..tree.child_count(node)).for_each(|index| {
        ...
        if child_style.position().is_out_of_flow() {
            ...
            /// Compute the static-position anchor and edge for a single axis
            fn static_position_for_axis(alignment: AlignSelf, area: Line<f32>, axis_is_rtl: bool) -> StaticPosition {
                let anchor_for = |keyword: AlignItemsKeyword| {
                    ...
                    match keyword {
                        AlignItemsKeyword::Center => ((area.start + area.end) / 2.0, StaticEdge::Center),
                        _ if start_position => (area.start, StaticEdge::Start),
                        _ => (area.end, StaticEdge::End),
                    }
                };
                let (anchor, edge) = anchor_for(alignment.keyword);
                if matches!(alignment.safety, crate::style::AlignmentSafety::Safe) {
                    let (fallback_anchor, fallback_edge) =
                        anchor_for(crate::compute::common::alignment::resolve_self_alignment_safety(alignment, true));
                    StaticPosition {
                        anchor,
                        edge,
                        fallback_anchor,
                        fallback_edge,
                        safe_extent: Some(area.end - area.start),
                    }
                } else {
                    StaticPosition::from_edge(anchor, edge)
                }
            }
