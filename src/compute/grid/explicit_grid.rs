...
/// Returns the number of tracks contributed by a fixed-count `repeat()` fragment.
///
/// The multiplication saturates so invalid or extreme templates cannot overflow the
/// `u16` track-count representation used throughout grid sizing.
fn fixed_repeat_track_count(count: u16, repeated_track_count: u16) -> u16 {
    count.saturating_mul(repeated_track_count)
}

/// Sums track-count contributions while clamping at `u16::MAX`.
///
/// This keeps explicit-grid sizing resilient when several fragments, including
/// expanded fixed repeats, would otherwise overflow the internal track count.
fn saturating_track_count_sum(track_counts: impl Iterator<Item = u16>) -> u16 {
    track_counts.fold(0, u16::saturating_add)
}
...
pub(crate) fn compute_explicit_grid_size_in_axis(
    ...
    // This is the number of explicit grid tracks contributed by the non-auto parts of the explicit track list:
    // standalone track sizes plus fixed-count repeat() fragments, but excluding the auto-repeat clause.
    // Spec: explicit grid https://www.w3.org/TR/css-grid-2/#explicit-grid,
    // track list https://www.w3.org/TR/css-grid-2/#track-list,
    // fixed-repeat https://www.w3.org/TR/css-grid-2/#typedef-fixed-repeat,
    // auto-repeat https://www.w3.org/TR/css-grid-2/#typedef-auto-repeat.
    let non_auto_repeating_track_count =
        saturating_track_count_sum(template.clone().map(|track_def| match track_def {
            GenericGridTemplateComponent::Single(_) => 1,
            GenericGridTemplateComponent::Repeat(repeat) => match repeat.count() {
                RepetitionCount::Count(count) => fixed_repeat_track_count(count, repeat.track_count()),
    ...
    let grid_template_track_count = non_auto_repeating_track_count
        .saturating_add(fixed_repeat_track_count(num_repetitions, repetition_track_count));
...
pub(super) fn initialize_grid_tracks(
    ...
    let non_auto_repeating_track_count = track_template
        .clone()
        .map(|track_template| {
            saturating_track_count_sum(track_template.map(|track_def| match track_def {
                GenericGridTemplateComponent::Single(_) => 1,
                GenericGridTemplateComponent::Repeat(repeat) => match repeat.count() {
                    RepetitionCount::Count(count) => fixed_repeat_track_count(count, repeat.track_count()),
                    RepetitionCount::AutoFit | RepetitionCount::AutoFill => 0,
    ...
    // Materialize the explicit grid track-by-track from the explicit track list:
    // a single track sizing function yields one explicit grid track, fixed repeat() expands into its repeated
    // fragment, and auto-repeat fills the remaining explicit-grid slots computed earlier.
    // Spec: explicit grid https://www.w3.org/TR/css-grid-2/#explicit-grid,
    // track list https://www.w3.org/TR/css-grid-2/#track-list,
    // repeat() https://www.w3.org/TR/css-grid-2/#repeat-notation.
                ...
                match track_sizing_function {
                    ...
                    GenericGridTemplateComponent::Repeat(repeat) => match repeat.count() {
                        RepetitionCount::Count(count) => {
                            ...
                            let track_iter =
                                track_iter.cycle().take(fixed_repeat_track_count(count, repeat.track_count()) as usize);
                        ...
                        RepetitionCount::AutoFit | RepetitionCount::AutoFill => {
                            ...
                            // Spec: explicit grid https://www.w3.org/TR/css-grid-2/#explicit-grid,
                            // auto-track-list grammar https://www.w3.org/TR/css-grid-2/#typedef-auto-track-list,
                            // auto-repeat grammar https://www.w3.org/TR/css-grid-2/#typedef-auto-repeat,
                            // auto-fill/auto-fit expansion https://www.w3.org/TR/css-grid-2/#auto-repeat.
                            ...
                            for track_def in iter.take(auto_repeated_track_count) {
                                ...
                                if repeat.count() == RepetitionCount::AutoFit && !track_has_items(current_track_index) {
                                    // Empty auto-fit tracks collapse after placement:
                                    // auto-fit https://www.w3.org/TR/css-grid-2/#valdef-repeat-auto-fit,
                                    // collapsed track https://www.w3.org/TR/css-grid-2/#collapsed-grid-track,
                                    // collapsed gutter https://www.w3.org/TR/css-grid-2/#collapsed-gutter.
                                    // In this representation the gutter on the start side is the last element
                                    // already pushed into `tracks`, and the gutter on the end side is `gutter` below.
                                    if current_track_index + 1 == counts.len() {
                                        for previous_track in tracks.iter_mut().rev() {
                                            if previous_track.kind == super::types::GridTrackKind::Track
                                                && !previous_track.is_collapsed
                                            {
                                                break;
                                            }

                                            if previous_track.kind == super::types::GridTrackKind::Gutter {
    ...
    // The explicit grid starts after the negative implicit tracks and extends for `counts.explicit` tracks.
    // Any remainder here is the part of that explicit grid not yet materialized from the explicit track list,
    // which must be filled with auto tracks before the positive implicit grid begins.
    // Spec: explicit grid https://www.w3.org/TR/css-grid-2/#explicit-grid,
    // implicit grid tracks https://www.w3.org/TR/css-grid-2/#implicit-grid-track,
    // implicit-grid growth from explicit bounds https://www.w3.org/TR/css-grid-2/#auto-placement-algo.
    let explicit_grid_end = counts.negative_implicit.saturating_add(counts.explicit);
    let current_track_index = current_track_index.min(u16::MAX as usize) as u16;
    let grid_area_tracks = explicit_grid_end.saturating_sub(current_track_index);
