...
/// The edge of an out-of-flow box which its static-position anchor refers to (per-axis).
///
...
/// position is recorded as an *anchor* point plus the edge of the box that should be placed at
/// that anchor.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub enum StaticEdge {
    /// The anchor refers to the start (left/top) edge of the box's margin box
    Start,
    /// The anchor refers to the center of the box
    Center,
    /// The anchor refers to the end (right/bottom) edge of the box's margin box
    End,
}

/// The static position of an out-of-flow box in a single axis.
///
/// Anchors are initially relative to the border box of the node which produced the candidate and
/// are translated by each node's location as the candidate bubbles up the tree, so they are
/// always relative to the border box of the node currently holding the candidate.
#[derive(Debug, Copy, Clone, PartialEq)]
pub struct StaticPosition {
    /// The static-position anchor coordinate
    pub anchor: f32,
    /// Which edge of the box the anchor refers to
    pub edge: StaticEdge,
    /// The anchor coordinate to fall back to if the box overflows `safe_extent`
    pub fallback_anchor: f32,
    /// Which edge of the box `fallback_anchor` refers to
    pub fallback_edge: StaticEdge,
    /// If set, the extent of the alignment container in this axis: when the box's margin box is
    /// larger than this extent the `fallback_anchor`/`fallback_edge` pair is used instead
    /// (the `safe` overflow-position keyword from CSS Box Alignment)
    pub safe_extent: Option<f32>,
}

impl StaticPosition {
    /// Create a `StaticPosition` from an anchor and edge with no safe fallback
    pub const fn from_edge(anchor: f32, edge: StaticEdge) -> Self {
        Self { anchor, edge, fallback_anchor: anchor, fallback_edge: edge, safe_extent: None }
    }
}
...
impl OofCandidates {
    ...
    /// Translate the static-position anchors of every candidate in the list by `offset`.
    ///
    /// This is used to convert anchors from being relative to a child node's border box to being
    ...
    pub fn translate(&mut self, offset: Point<f32>) {
        for candidate in self.as_mut_slice() {
            candidate.static_position.x.anchor += offset.x;
            candidate.static_position.x.fallback_anchor += offset.x;
            candidate.static_position.y.anchor += offset.y;
            candidate.static_position.y.fallback_anchor += offset.y;
        }
    }
}
