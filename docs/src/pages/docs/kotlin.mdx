...
The `Display` property specifies wether the component should be laid out as `Flex` or not at all. If `None` is specified then the node will have a resulting size of zero. In the future this enum class will be expanded to include more display options such as `Grid`.
