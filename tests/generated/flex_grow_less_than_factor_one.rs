...
fn flex_grow_less_than_factor_one() {
    ...
    let node1 = taffy
        .new_with_children(
            taffy::style::FlexboxLayout { flex_grow: 0.2f32, flex_shrink: 0f32, ..Default::default() },
            &[],
        )
        .unwrap();
    let node2 = taffy
        .new_with_children(
            taffy::style::FlexboxLayout { flex_grow: 0.4f32, flex_shrink: 0f32, ..Default::default() },
            &[],
        )
        .unwrap();
    let node = taffy
