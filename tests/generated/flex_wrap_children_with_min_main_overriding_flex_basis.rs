...
fn flex_wrap_children_with_min_main_overriding_flex_basis() {
    ...
    let node0 = stretch.new_node(
        stretch::style::Style {
            ...
            min_size: stretch::geometry::Size { width: stretch::style::Dimension::Points(55f32), ..Default::default() },
            ..Default::default()
        },
        vec![],
    );
    ...
    assert_eq!(stretch.layout(node1).size.width, 55f32);
    assert_eq!(stretch.layout(node1).size.height, 50f32);
    assert_eq!(stretch.layout(node1).location.x, 0f32);
    assert_eq!(stretch.layout(node1).location.y, 50f32);
}
