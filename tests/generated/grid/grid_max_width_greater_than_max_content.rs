...
fn grid_max_width_greater_than_max_content__border_box() {
    ...
    if layout.size.width != 80f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node00, stringify!(size.width), 80f32, layout.size.width);
    }
    if layout.size.height != 10f32 {
        mismatches += 1;
        eprintln!(
    ...
    if layout.location.x != 80f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node01, stringify!(location.x), 80f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
