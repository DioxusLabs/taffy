...
fn grid_taffy_issue_624__border_box() {
    ...
    if layout.size.height != 640f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node, stringify!(size.height), 640f32, layout.size.height);
    }
    ...
    if layout.size.width != 120f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node2, stringify!(size.width), 120f32, layout.size.width);
    }
