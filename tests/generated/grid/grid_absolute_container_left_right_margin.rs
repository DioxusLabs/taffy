...
fn grid_absolute_container_left_right_margin__border_box() {
    ...
    if layout.size.width != 167f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.width), 167f32, layout.size.width);
    }
    ...
    if layout.location.x != 9f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(location.x), 9f32, layout.location.x);
    }
