...
fn grid_margins_percent_end__border_box() {
    ...
    if layout.location.x != -2f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(location.x), -2f32, layout.location.x);
    }
    ...
    if layout.location.x != 20f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node4, stringify!(location.x), 20f32, layout.location.x);
    }
    if layout.location.y != 40f32 {
        mismatches += 1;
