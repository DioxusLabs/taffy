...
fn grid_max_content_single_item_margin_percent__border_box() {
    ...
    if layout.size.width != 28f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(size.width), 28f32, layout.size.width);
    }
    ...
    if layout.location.y != 2f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.y), 2f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
