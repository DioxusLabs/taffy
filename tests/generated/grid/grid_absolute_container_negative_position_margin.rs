...
fn grid_absolute_container_negative_position_margin__border_box() {
    ...
    if layout.location.x != 193f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(location.x), 193f32, layout.location.x);
    }
    ...
    if layout.location.y != 182f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.y), 182f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
