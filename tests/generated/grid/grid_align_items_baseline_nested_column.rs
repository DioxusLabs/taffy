...
fn grid_align_items_baseline_nested_column__border_box() {
    ...
    if layout.size.width != 50f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node101, stringify!(size.width), 50f32, layout.size.width);
    }
    ...
    if layout.location.x != 0f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node101, stringify!(location.x), 0f32, layout.location.x);
    }
    if layout.location.y != 35f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node101, stringify!(location.y), 35f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
