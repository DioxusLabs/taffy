...
fn grid_absolute_column_end__border_box_rtl() {
    ...
    let layout = taffy.layout(node0).unwrap();
    let Layout { size, location, .. } = layout;
    assert_eq!(size.width, 13f32, "width of node {:?}. Expected {}. Actual {}", node0, 13f32, size.width);
    assert_eq!(size.height, 157f32, "height of node {:?}. Expected {}. Actual {}", node0, 157f32, size.height);
    assert_eq!(location.x, 164f32, "x of node {:?}. Expected {}. Actual {}", node0, 164f32, location.x);
    assert_eq!(location.y, 1f32, "y of node {:?}. Expected {}. Actual {}", node0, 1f32, location.y);
}
