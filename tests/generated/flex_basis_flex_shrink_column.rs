...
fn flex_basis_flex_shrink_column() {
    ...
    let node0 = taffy
        .new_leaf(taffy::style::Style { flex_basis: taffy::style::Dimension::Points(100f32), ..Default::default() })
        .unwrap();
    let node1 = taffy
        .new_leaf(taffy::style::Style { flex_basis: taffy::style::Dimension::Points(50f32), ..Default::default() })
        .unwrap();
    let node = taffy
        .new_with_children(
            taffy::style::Style {
    ...
    assert_eq!(size.height, 33f32, "height of node {:?}. Expected {}. Actual {}", node1.data(), 33f32, size.height);
    assert_eq!(location.x, 0f32, "x of node {:?}. Expected {}. Actual {}", node1.data(), 0f32, location.x);
    assert_eq!(location.y, 67f32, "y of node {:?}. Expected {}. Actual {}", node1.data(), 67f32, location.y);
}
