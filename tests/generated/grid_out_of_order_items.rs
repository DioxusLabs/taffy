...
fn grid_out_of_order_items() {
    ...
    assert_eq!(taffy.layout(node2).unwrap().size.width, 35f32);
    assert_eq!(taffy.layout(node2).unwrap().size.height, 35f32);
    ...
    assert_eq!(taffy.layout(node7).unwrap().size.width, 10f32);
    assert_eq!(taffy.layout(node7).unwrap().size.height, 10f32);
    assert_eq!(taffy.layout(node7).unwrap().location.x, 40f32);
    assert_eq!(taffy.layout(node7).unwrap().location.y, 0f32);
