...
fn grid_margins_fixed_end() {
    ...
    assert_eq!(location.x, 58f32, "x of node {:?}. Expected {}. Actual {}", node0.data(), 58f32, location.x);
    assert_eq!(location.y, 27f32, "y of node {:?}. Expected {}. Actual {}", node0.data(), 27f32, location.y);
