...
fn justify_content_column_space_around() {
    ...
    let node = stretch
        .new_node(
            stretch2::style::Style {
                ...
                justify_content: stretch2::style::JustifyContent::SpaceAround,
                size: stretch2::geometry::Size {
    ...
    assert_eq!(stretch.layout(node0).unwrap().location.y, 12f32);
    ...
    assert_eq!(stretch.layout(node1).unwrap().location.y, 45f32);
    ...
    assert_eq!(stretch.layout(node2).unwrap().location.y, 78f32);
}
