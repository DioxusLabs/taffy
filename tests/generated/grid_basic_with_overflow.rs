...
fn grid_basic_with_overflow() {
    ...
    assert_eq!(taffy.layout(node9).unwrap().size.width, 40f32);
    assert_eq!(taffy.layout(node9).unwrap().size.height, 0f32);
    assert_eq!(taffy.layout(node9).unwrap().location.x, 0f32);
    assert_eq!(taffy.layout(node9).unwrap().location.y, 120f32);
}
