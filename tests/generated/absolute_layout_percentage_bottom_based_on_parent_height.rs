...
fn absolute_layout_percentage_bottom_based_on_parent_height() {
    ...
    let node0 = stretch.new_node(
        stretch::style::Style {
            ...
            position: stretch::geometry::Rect { top: stretch::style::Dimension::Percent(0.5f32), ..Default::default() },
            ..Default::default()
        },
        vec![],
    );
    ...
    assert_eq!(stretch.layout(node0).location.y, 100f32);
    ...
    assert_eq!(stretch.layout(node1).location.y, 90f32);
    assert_eq!(stretch.layout(node2).size.width, 10f32);
    assert_eq!(stretch.layout(node2).size.height, 160f32);
