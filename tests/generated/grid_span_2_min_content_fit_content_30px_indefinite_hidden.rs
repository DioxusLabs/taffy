#[test]
fn grid_span_2_min_content_fit_content_30px_indefinite_hidden() {
    use slotmap::Key;
    ...
    let node = taffy
        .new_with_children(
            taffy::style::Style {
                ...
                grid_template_columns: vec![min_content(), fit_content(points(30f32))],
                ..Default::default()
            },
            &[node0, node1, node2],
        )
    ...
    assert_eq!(size.width, 70f32, "width of node {:?}. Expected {}. Actual {}", node.data(), 70f32, size.width);
    ...
    assert_eq!(size.width, 70f32, "width of node {:?}. Expected {}. Actual {}", node2.data(), 70f32, size.width);
