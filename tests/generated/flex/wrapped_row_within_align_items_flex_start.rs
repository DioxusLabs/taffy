...
fn wrapped_row_within_align_items_flex_start__border_box() {
    ...
    if layout.size.width != 150f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node00, stringify!(size.width), 150f32, layout.size.width);
    }
    ...
    if layout.size.width != 80f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node01, stringify!(size.width), 80f32, layout.size.width);
    }
    if layout.size.height != 80f32 {
        mismatches += 1;
        eprintln!(
    ...
    if layout.location.y != 80f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node01, stringify!(location.y), 80f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
