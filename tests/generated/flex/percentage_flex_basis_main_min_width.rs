...
fn percentage_flex_basis_main_min_width__border_box() {
    ...
    let node0 = taffy
        .new_leaf(taffy::style::Style {
            ...
            min_size: taffy::geometry::Size { width: taffy::style::Dimension::from_percent(0.6f32), height: auto() },
            ..Default::default()
        })
        .unwrap();
    let node1 = taffy
    ...
    if layout.size.width != 80f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(size.width), 80f32, layout.size.width);
    }
    if layout.size.height != 400f32 {
        mismatches += 1;
        eprintln!(
