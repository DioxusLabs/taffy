...
fn percentage_flex_basis_main_max_height__border_box() {
    ...
    let node0 = taffy
        .new_leaf(taffy::style::Style {
            ...
            max_size: taffy::geometry::Size { width: auto(), height: taffy::style::Dimension::from_percent(0.6f32) },
            ..Default::default()
        })
        .unwrap();
    let node1 = taffy
        .new_leaf(taffy::style::Style {
            ...
            flex_grow: 4f32,
            flex_basis: taffy::style::Dimension::from_percent(0.1f32),
            max_size: taffy::geometry::Size { width: auto(), height: taffy::style::Dimension::from_percent(0.2f32) },
            ..Default::default()
        })
        .unwrap();
    let node = taffy
    ...
    if layout.size.width != 52f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.width), 52f32, layout.size.width);
    }
    ...
    if layout.location.x != 52f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.x), 52f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
