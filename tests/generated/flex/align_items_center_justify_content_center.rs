...
fn align_items_center_justify_content_center__border_box() {
    ...
    if layout.size.width != 500f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.width), 500f32, layout.size.width);
    }
    ...
    if layout.location.x != 245f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node00, stringify!(location.x), 245f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
    ...
    if layout.location.y != 20f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node000, stringify!(location.y), 20f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
