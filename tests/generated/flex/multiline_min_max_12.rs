...
fn multiline_min_max_12__border_box() {
    ...
    let node0 = taffy
        .new_leaf(taffy::style::Style {
            ...
            padding: taffy::geometry::Rect { left: length(10f32), right: zero(), top: zero(), bottom: zero() },
            ..Default::default()
        })
        .unwrap();
    let node1 = taffy
    ...
    if layout.location.x != 305f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.x), 305f32, layout.location.x);
    }
    if layout.location.y != 5f32 {
        mismatches += 1;
    ...
    if layout.location.x != 405f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node2, stringify!(location.x), 405f32, layout.location.x);
    }
    if layout.location.y != 5f32 {
        mismatches += 1;
    ...
    if layout.location.x != 505f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node3, stringify!(location.x), 505f32, layout.location.x);
    }
    if layout.location.y != 5f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node3, stringify!(location.y), 5f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
