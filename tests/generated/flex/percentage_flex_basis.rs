...
fn percentage_flex_basis__border_box() {
    ...
    if layout.size.width != 125f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.width), 125f32, layout.size.width);
    }
    if layout.size.height != 200f32 {
        mismatches += 1;
        eprintln!(
    ...
    if layout.location.x != 125f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.x), 125f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
