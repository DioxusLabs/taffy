...
fn bevy_issue_16304__border_box() {
    ...
    if layout.size.width != 20f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node001, stringify!(size.width), 20f32, layout.size.width);
    }
    if layout.size.height != 40f32 {
        mismatches += 1;
        eprintln!(
    ...
    if layout.location.x != 30f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node001, stringify!(location.x), 30f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
