...
fn wrap_nodes_with_content_sizing_margin_cross__border_box() {
    ...
    let node0 = taffy
        .new_with_children(
            taffy::style::Style {
                ...
                size: taffy::geometry::Size { width: taffy::style::Dimension::from_length(70f32), height: auto() },
                ..Default::default()
            },
            &[node00, node01],
        )
    ...
    if layout.location.y != 50f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node01, stringify!(location.y), 50f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
