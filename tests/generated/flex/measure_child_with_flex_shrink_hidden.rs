...
fn measure_child_with_flex_shrink_hidden__border_box() {
    ...
    if layout.size.width != 91f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(size.width), 91f32, layout.size.width);
    }
    if layout.size.height != 50f32 {
        mismatches += 1;
    ...
    if layout.location.x != 9f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.x), 9f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
