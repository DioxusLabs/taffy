...
fn position_root_with_rtl_should_position_withoutdirection__border_box() {
    ...
    if layout.size.height != 52f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.height), 52f32, layout.size.height);
    }
    if layout.location.x != 72f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(location.x), 72f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
