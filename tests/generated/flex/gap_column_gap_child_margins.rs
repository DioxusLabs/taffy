...
fn gap_column_gap_child_margins__border_box() {
    ...
    if layout.location.x != 26f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.x), 26f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
    ...
    if layout.size.width != 2f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node2, stringify!(size.width), 2f32, layout.size.width);
    }
    if layout.size.height != 100f32 {
        mismatches += 1;
        eprintln!(
