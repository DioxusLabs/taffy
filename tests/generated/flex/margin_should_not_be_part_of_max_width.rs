...
fn margin_should_not_be_part_of_max_width__border_box() {
    ...
    let node0 = taffy
        .new_leaf(taffy::style::Style {
            ...
            margin: taffy::geometry::Rect { left: length(20f32), right: zero(), top: zero(), bottom: zero() },
            ..Default::default()
        })
        .unwrap();
    let node = taffy
    ...
    if layout.size.width != 250f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node, stringify!(size.width), 250f32, layout.size.width);
    }
    if layout.size.height != 250f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node, stringify!(size.height), 250f32, layout.size.height);
    }
