...
fn percentage_position_bottom_right__border_box() {
    ...
    if layout.size.width != 275f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.width), 275f32, layout.size.width);
    }
    if layout.size.height != 75f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.height), 75f32, layout.size.height);
    }
