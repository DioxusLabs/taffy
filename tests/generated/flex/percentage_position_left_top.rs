...
fn percentage_position_left_top__border_box() {
    ...
    let node0 = taffy
        .new_leaf(taffy::style::Style {
            ...
            size: taffy::geometry::Size {
                width: taffy::style::Dimension::from_percent(0.45f32),
                height: taffy::style::Dimension::from_percent(0.55f32),
            },
            inset: taffy::geometry::Rect { left: percent(0.1f32), right: auto(), top: percent(0.2f32), bottom: auto() },
            ..Default::default()
        })
        .unwrap();
    let node = taffy
    ...
    if layout.size.width != 180f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.width), 180f32, layout.size.width);
    }
    if layout.size.height != 220f32 {
        mismatches += 1;
        eprintln!(
            "{:?}.{} mismatch: expected {} actual {}",
            node0,
