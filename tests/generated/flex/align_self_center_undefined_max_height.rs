...
fn align_self_center_undefined_max_height__border_box() {
    ...
    let node1 = taffy
        .new_leaf(taffy::style::Style {
            ...
            size: taffy::geometry::Size {
                ...
                height: taffy::style::Dimension::from_length(56f32),
            },
            ..Default::default()
        })
        .unwrap();
    ...
    if layout.size.width != 280f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node, stringify!(size.width), 280f32, layout.size.width);
    }
    ...
    if layout.size.width != 240f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.width), 240f32, layout.size.width);
    }
    ...
    if layout.size.height != 56f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(size.height), 56f32, layout.size.height);
    }
    if layout.location.x != 240f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.x), 240f32, layout.location.x);
    }
    if layout.location.y != 0f32 {
        mismatches += 1;
