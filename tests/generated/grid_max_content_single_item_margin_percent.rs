...
fn grid_max_content_single_item_margin_percent() {
    ...
    assert_eq!(taffy.layout(node1).unwrap().size.width, 28f32);
    assert_eq!(taffy.layout(node1).unwrap().size.height, 32f32);
    assert_eq!(taffy.layout(node1).unwrap().location.x, 48f32);
    assert_eq!(taffy.layout(node1).unwrap().location.y, 2f32);
