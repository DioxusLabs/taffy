...
fn percentage_multiple_nested_with_padding_margin_and_percentage_values() {
    ...
    let node000 = stretch.new_node(
        stretch::style::Style {
            size: stretch::geometry::Size { width: stretch::style::Dimension::Percent(0.45f32), ..Default::default() },
    ...
    let node00 = stretch.new_node(
        stretch::style::Style {
            ...
            size: stretch::geometry::Size { width: stretch::style::Dimension::Percent(0.5f32), ..Default::default() },
            margin: stretch::geometry::Rect {
    ...
    assert_eq!(stretch.layout(node0).size.width, 190f32);
    assert_eq!(stretch.layout(node0).size.height, 48f32);
    assert_eq!(stretch.layout(node0).location.x, 5f32);
    assert_eq!(stretch.layout(node0).location.y, 5f32);
    assert_eq!(stretch.layout(node00).size.width, 92f32);
    assert_eq!(stretch.layout(node00).size.height, 25f32);
    assert_eq!(stretch.layout(node00).location.x, 8f32);
    assert_eq!(stretch.layout(node00).location.y, 8f32);
    assert_eq!(stretch.layout(node000).size.width, 36f32);
    assert_eq!(stretch.layout(node000).size.height, 6f32);
    assert_eq!(stretch.layout(node000).location.x, 10f32);
    assert_eq!(stretch.layout(node000).location.y, 10f32);
    assert_eq!(stretch.layout(node1).size.width, 200f32);
    assert_eq!(stretch.layout(node1).size.height, 142f32);
    assert_eq!(stretch.layout(node1).location.x, 0f32);
    assert_eq!(stretch.layout(node1).location.y, 58f32);
}
