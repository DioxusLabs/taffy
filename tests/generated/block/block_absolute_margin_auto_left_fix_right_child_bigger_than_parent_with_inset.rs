...
fn block_absolute_margin_auto_left_fix_right_child_bigger_than_parent_with_inset__border_box() {
    ...
    if layout.size.width != 52f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node, stringify!(size.width), 52f32, layout.size.width);
    }
    ...
    if layout.size.width != 72f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.width), 72f32, layout.size.width);
    }
    if layout.size.height != 72f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.height), 72f32, layout.size.height);
    }
