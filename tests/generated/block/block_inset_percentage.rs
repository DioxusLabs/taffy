...
fn block_inset_percentage__border_box() {
    ...
    if layout.location.x != -3f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.x), -3f32, layout.location.x);
    }
    ...
    if layout.location.x != 1f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node2, stringify!(location.x), 1f32, layout.location.x);
    }
