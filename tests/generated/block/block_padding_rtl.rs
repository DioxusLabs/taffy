    ...
    let node = taffy
        .new_with_children(
            taffy::style::Style {
                ...
                border: taffy::geometry::Rect { left: length(5f32), right: length(15f32), top: zero(), bottom: zero() },
                ..Default::default()
            },
            &[node0],
        )
...
fn block_padding_rtl__content_box_ltr() {
    ...
    assert_eq!(size.width, 20f32, "width of node {:?}. Expected {}. Actual {}", node0, 20f32, size.width);
    assert_eq!(size.height, 10f32, "height of node {:?}. Expected {}. Actual {}", node0, 10f32, size.height);
    assert_eq!(location.x, 95f32, "x of node {:?}. Expected {}. Actual {}", node0, 95f32, location.x);
    assert_eq!(location.y, 0f32, "y of node {:?}. Expected {}. Actual {}", node0, 0f32, location.y);
}
