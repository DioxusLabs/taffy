...
fn block_margin_y_collapse_through_positive_and_negative__border_box() {
    ...
    if layout.size.height != 17f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node, stringify!(size.height), 17f32, layout.size.height);
    }
    ...
    if layout.location.y != 7f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node2, stringify!(location.y), 7f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
