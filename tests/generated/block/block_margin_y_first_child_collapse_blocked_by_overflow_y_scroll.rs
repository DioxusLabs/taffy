...
fn block_margin_y_first_child_collapse_blocked_by_overflow_y_scroll__border_box() {
    ...
    if layout.size.width != 35f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node00, stringify!(size.width), 35f32, layout.size.width);
    }
    ...
    if layout.size.width != 35f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node000, stringify!(size.width), 35f32, layout.size.width);
    }
    if layout.size.height != 10f32 {
        mismatches += 1;
        eprintln!(
