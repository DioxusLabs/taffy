...
fn block_margin_y_collapse_through_blocked_by_min_height__border_box() {
    ...
    let node1 = taffy
        .new_leaf(taffy::style::Style {
            display: taffy::style::Display::Block,
            min_size: taffy::geometry::Size { width: auto(), height: taffy::style::Dimension::from_length(1f32) },
    ...
    if layout.size.height != 41f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node, stringify!(size.height), 41f32, layout.size.height);
    }
    ...
    if layout.size.height != 1f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(size.height), 1f32, layout.size.height);
    }
    if layout.location.x != 0f32 {
        mismatches += 1;
    ...
    if layout.location.y != 31f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node2, stringify!(location.y), 31f32, layout.location.y);
    }
    #[cfg(feature = "content_size")]
    if layout.scroll_width() != 0f32 {
        mismatches += 1;
