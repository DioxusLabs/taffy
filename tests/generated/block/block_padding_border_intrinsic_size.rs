...
fn block_padding_border_intrinsic_size__border_box() {
    ...
    if layout.size.height != 34f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node, stringify!(size.height), 34f32, layout.size.height);
    }
    ...
    if layout.location.x != 15f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node1, stringify!(location.x), 15f32, layout.location.x);
    }
