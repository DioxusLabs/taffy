...
fn block_padding_border_percentage_fixed_size__border_box() {
    ...
    if layout.size.height != 12f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node0, stringify!(size.height), 12f32, layout.size.height);
    }
    if layout.location.x != 0f32 {
        mismatches += 1;
    ...
    if layout.size.width != 47f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node00, stringify!(size.width), 47f32, layout.size.width);
    }
    ...
    if layout.location.x != 2f32 {
        mismatches += 1;
        eprintln!("{:?}.{} mismatch: expected {} actual {}", node00, stringify!(location.x), 2f32, layout.location.x);
    }
