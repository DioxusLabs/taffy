...
mod detailed_grid_info {
    ...
    #[test]
    fn rtl_mirrors_column_line_names() {
        let mut tree = new_test_tree();
        let child = tree.new_leaf(Style::default()).unwrap();
        let root = tree
            .new_with_children(
        ...
        let info = get_detailed_grid_info(&tree, root);
        let column_lines: Vec<&[String]> = info.columns.line_names.iter().collect();
        // Columns are mirrored in RTL, so line names are mirrored to stay aligned with positions
        assert_eq!(column_lines[0], ["c".to_string()]);
        assert_eq!(column_lines[1], ["b".to_string()]);
        assert_eq!(column_lines[2], ["a".to_string()]);
        assert_eq!(info.grid_template_columns(), "[c] 60px [b] 40px [a]");
    }
}
